...
export default function QuadroEtaPage() {
  ...
  useEffect(() => {
    if (!isClient) return
    loadSoci()
  }, [isClient, etaMin, etaMax, fasce, filtroZona, filtroCirc, filtroClub, filtroDistretto])
      ...
      <motion.div variants={itemVariants} className="mb-6 print-hide">
        <Card className="border border-border/50 bg-card/50 backdrop-blur-sm">
          <CardContent className="pt-4">
            ...
            {(etaMin || etaMax || fasce.length > 0 || filtroZona.length > 0 || filtroCirc.length > 0 || filtroClub.length > 0 || filtroDistretto.length > 0) && (
              <Button variant="outline" size="sm" onClick={() => { setEtaMin(''); setEtaMax(''); setFasce([]); setFiltroZona([]); setFiltroCirc([]); setFiltroClub([]); setFiltroDistretto([]) }} className="text-xs">Cancella filtri</Button>
            )}
          </CardContent>
        </Card>
      </motion.div>
