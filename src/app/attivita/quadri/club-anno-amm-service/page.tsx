...
export default function QuadroClubAnnoAmmServicePage() {
  ...
  useEffect(() => {
    if (!isClient || !club) { setActivities([]); return }
    loadActivities()
  }, [isClient, club, annoSociale])
  ...
  async function loadClubs() {
    const { data } = await supabase
      ...
      .range(0, 9999)
    if (data) setClubs([...new Set(data.map((c: any) => c.sponsor_nome_account))].filter(Boolean).sort() as string[])
  }
  ...
  async function loadActivities() {
    ...
    const { data, error } = await supabase
      .from('vista_report_ricerca')
      .select('id_attivita, data_inizio, stato, titolo, causa, tipo_progetto, persone_servite_limite, totale_volontari, totale_ore_servizio_capped, fondi_donati_usd_capped, organizzazione_beneficiata, fondi_raccolti_usd_capped')
  ...
  return (
    <motion.main initial="hidden" animate="visible" variants={containerVariants} className="container mx-auto p-4 sm:p-8 print-area print-landscape">
      <motion.div variants={itemVariants} className="flex items-center justify-between mb-4 print-hide gap-2 flex-wrap">
        ...
        <Button onClick={() => window.print()} size="sm" className="text-xs gap-1.5" disabled={!club || activities.length === 0}>
      ...
      <motion.p variants={itemVariants} className="text-sm text-muted-foreground mb-6 print:text-black">
        {club ? <><strong className="text-foreground print:text-black">{club}</strong> · Anno sociale <strong className="text-foreground print:text-black">{annoLabel}</strong> · {activities.length} attività ({amministrazione.length} Amministrazione, {service.length} Service)</> : 'Seleziona un club per visualizzare le attività'}
      </motion.p>
          ...
          <CardContent className="print:p-0 space-y-8">
            {error ? (
              ...
              <div className="flex flex-col justify-center items-center h-32 gap-2 text-muted-foreground">
                ...
                <span className="text-sm">Seleziona un club dal pannello in alto</span>
              </div>
            ) : activities.length === 0 ? (
              <div className="flex flex-col justify-center items-center h-32 gap-2 text-muted-foreground">
                <Activity className="w-8 h-8 opacity-30" />
                <span className="text-sm">Nessuna attività per {club} nell&apos;anno sociale {annoLabel}</span>
              </div>
            ) : (
              <>
