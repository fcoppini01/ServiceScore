...
export default function QuadroClubAnnoPage() {
  ...
  async function loadActivities() {
    ...
    const { data, error } = await supabase
      .from('vista_report_ricerca')
      .select('id_attivita, data_inizio, stato, titolo, causa, tipo_progetto, persone_servite, totale_volontari, totale_ore_servizio_capped, fondi_donati_usd_capped, organizzazione_beneficiata, fondi_raccolti_usd_capped')
  ...
  return (
    <motion.main initial="hidden" animate="visible" variants={containerVariants} className="container mx-auto p-4 sm:p-8 print-area print-landscape">
      ...
      <motion.p variants={itemVariants} className="text-sm text-muted-foreground mb-6 print:text-black">
        {club ? <><strong className="text-foreground print:text-black">{club}</strong> · Anno sociale <strong className="text-foreground print:text-black">{annoLabel}</strong> · {activities.length} attività</> : 'Seleziona un club per visualizzare le attività'}
      </motion.p>
